...
<div style="width: 900px; margin: 0 auto;">
	...
	<br><br><br>
		...
		<div id="cafename"> 
			<p style="font-size: 50px; display: inline-block; margin: 5px 10px;">
				<%=viewCafeInfo.getCafe_name() %>
			</p>
			
			<p style="font-size: 30px; display: inline-block; margin-left: 5px; color:#FF792A;">
				<%=starAvg %>
			</p> 
		</div>
		...
		<%if(session.getAttribute("login")!=null && (boolean) session.getAttribute("login")){ %>
			<div  class="middle" >
				<button class="review" id="btnReview" onclick="location='/review/write?cafeinfo=<%=viewCafeInfo.getCafe_no() %>'"><b>리뷰 작성</b></button>
			</div>
		<%}%>
		...
		<div> 
			<span style="font-size: 18px; color:#888">&nbsp;&nbsp;&nbsp;♥ <%=viewCafeInfo.getCafe_rcm() %></span>
		</div>
			...
			<%if(viewCafeInfo.getCafe_no() ==cafeList.get(i).getCafe_no()){ %>
					...
					<div style="display: inline-block; float:left;">
						<%for(int j =0; j<cafereviewFile.size();j++){ 
							if(cafeList.get(i).getReview_no()==cafereviewFile.get(j).getReview_no()){%>
								<div id="review<%=j %>" class="modal"><img src="/upload/<%=cafereviewFile.get(j).getRvw_cpy_file_name() %>" class="cafeimg"  style="width:300px; height:300px;"></div>		
				...
				<%if(session.getAttribute("login")!=null && (boolean) session.getAttribute("login")){
					if(Integer.parseInt(String.valueOf(session.getAttribute("user_no"))) == cafeList.get(i).getUser_no() ){ %>
						<ul>
							<li>
								<form action="/review/delete?reviewno=<%=cafeList.get(i).getReview_no() %>" name="deleteform" method="post"></form>
							...
							<li style="margin-right: 5px;">
								<a class="delete" onclick="location='/review/update?reviewno=<%=cafeList.get(i).getReview_no() %>'" >수정</a>
							<li>
